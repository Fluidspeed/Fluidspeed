    ...
    IConstantFlowAgreementV1
{
    ...
    bytes32 private constant CFAV1_PPP_CONFIG_KEY =
        keccak256("org.fluidspeed-finance.agreements.ConstantFlowAgreement.v1.PPPConfiguration");

    bytes32 private constant SUPERTOKEN_MINIMUM_DEPOSIT_KEY =
        keccak256("org.fluidspeed-finance.fluidspeed.superTokenMinimumDeposit");
    ...
    constructor(IFluidspeed host) AgreementBase(address(host)) {}
     ...
     *************************************************************************/
         ...
         returns (uint256 deposit)
     {
        ...
        if (flowRate == 0) return 0;
         ...
         (uint256 liquidationPeriod, ) = FluidspeedGovernanceConfigs.decodePPPConfig(pppConfig);
    ...
    {
        IFluidspeed.Context memory currentContext = AgreementLibrary.authorizeTokenAccess(token, ctx);
    ...
    function _createOrUpdateFlowCheck(
        _StackVars_createOrUpdateFlow memory flowVars,
        IFluidspeed.Context memory currentContext
    )
        internal pure
        returns(bytes32 flowId, FlowParams memory flowParams)
    {
        if (flowVars.receiver == address(0)) {
            revert FluidspeedErrors.ZERO_ADDRESS(FluidspeedErrors.CFA_ZERO_ADDRESS_RECEIVER);
        }
        ...
        returns(bytes memory newCtx)
    {
        ...
        (bool exist, FlowData memory oldFlowData) = _getAgreementData(flowVars.token, flowId);
        if (exist) revert FluidspeedErrors.ALREADY_EXISTS(FluidspeedErrors.CFA_FLOW_ALREADY_EXISTS);

        if (IFluidspeed(msg.sender).isApp(ISuperApp(flowVars.receiver))) {
        ...
        returns(bytes memory newCtx)
    {
        ...
        if (!exist) revert FluidspeedErrors.DOES_NOT_EXIST(FluidspeedErrors.CFA_FLOW_DOES_NOT_EXIST);
        ...
        returns(bytes memory newCtx)
    {
        ...
        if (flowVars.sender == address(0)) {
            revert FluidspeedErrors.ZERO_ADDRESS(FluidspeedErrors.CFA_ZERO_ADDRESS_SENDER);
        }
        if (flowVars.receiver == address(0)) {
            revert FluidspeedErrors.ZERO_ADDRESS(FluidspeedErrors.CFA_ZERO_ADDRESS_RECEIVER);
        }
        ...
        if (currentContext.msgSender != flowVars.sender &&
            ...
            !hasPermissions)
        {
            if (!IFluidspeed(msg.sender).isAppJailed(ISuperApp(flowVars.sender)) &&
                !IFluidspeed(msg.sender).isAppJailed(ISuperApp(flowVars.receiver))) {
        ...
        // if the receiver of the flow is deleting the flow
            ...
            if (IFluidspeed(msg.sender).isApp(ISuperApp(flowVars.sender))) {
                newCtx = _changeFlowToApp(
                    flowVars.sender,
                    flowVars.token, flowParams, oldFlowData,
                    newCtx, currentContext, FlowChangeType.DELETE_FLOW);
            // if the receiver of the flow deleting the flow is a super app
            } else if (IFluidspeed(msg.sender).isApp(ISuperApp(flowVars.receiver))) {
                newCtx = _changeFlowToApp(
                    address(0),
                    flowVars.token, flowParams, oldFlowData,
                    newCtx, currentContext, FlowChangeType.DELETE_FLOW);
        ...
        // flowOperator case OR liquidation case (when the msgSender isn't the sender or receiver)
            ...
            // we jail the app
            if (IFluidspeed(msg.sender).isApp(ISuperApp(flowVars.sender)) && availableBalance < 0) {
                newCtx = IFluidspeed(msg.sender).jailApp(
                    newCtx,
        ...
        returns (bytes memory newCtx)
    {
        ...
        if (currentContext.appCreditToken == token) {
            newCtx = IFluidspeed(msg.sender).ctxUseCredit(
                ctx,
                depositDelta
            );
        } else {
    ...
    struct _StackVars_changeFlowToApp {
        ...
        IFluidspeed.Context appContext;
    }
        ...
        returns (bytes memory newCtx)
    {
        ...
        if (appToCallback != address(0)) {
            ...
            IFluidspeedGovernance gov = IFluidspeedGovernance(IFluidspeed(msg.sender).getGovernance());
            ...
            // Rule CFA-2
            // https://github.com/fluidspeed-finance/protocol-monorepo/wiki/About-App-Credit
            // Allow apps to take an additional amount of app credit (minimum deposit)
            uint256 minimumDeposit = gov.getConfigAsUint256(
                IFluidspeed(msg.sender), token, SUPERTOKEN_MINIMUM_DEPOSIT_KEY);
        ...
        // and its range will be [0, currentContext.appCreditGranted]
        {
            ...
            if (IFluidspeed(msg.sender).isApp(ISuperApp(flowParams.receiver))) {
                ...
                if (availableBalance < 0) {
                    ...
                    if (optype == FlowChangeType.DELETE_FLOW) {
                        ...
                        revert FluidspeedErrors.APP_RULE(SuperAppDefinitions.APP_RULE_NO_CRITICAL_RECEIVER_ACCOUNT);
                    }
                }
            }
        }
        ...
        private view
    {
        ...
        if (currentContext.callType != ContextDefinitions.CALL_INFO_CALL_TYPE_APP_CALLBACK ||
            ...
            if (availableBalance < 0) {
                revert FluidspeedErrors.INSUFFICIENT_BALANCE(FluidspeedErrors.CFA_INSUFFICIENT_BALANCE);
            }
        }
    }
