    ...
    IInstantDistributionAgreementV1
{
    ...
    {
        IFluidspeed.Context memory context = AgreementLibrary.authorizeTokenAccess(token, ctx);
        address publisher = context.msgSender;
        bytes32 iId = _getPublisherId(publisher, indexId);
        if (_hasIndexData(token, iId)) {
            revert FluidspeedErrors.ALREADY_EXISTS(FluidspeedErrors.IDA_INDEX_ALREADY_EXISTS);
        }
            ...
            uint128 newIndexValue)
    {
        ...
        if (!exist) revert FluidspeedErrors.DOES_NOT_EXIST(FluidspeedErrors.IDA_INDEX_DOES_NOT_EXIST);
        ...
        private
    {
        ...
        if (token.isAccountCriticalNow(publisher)) {
            revert FluidspeedErrors.INSUFFICIENT_BALANCE(FluidspeedErrors.IDA_INSUFFICIENT_BALANCE);
        }
    }
        ...
        returns(bytes memory newCtx)
    {
        ...
        if (vars.subscriptionExists) {
            ...
            if (vars.sdata.subId != _UNALLOCATED_SUB_ID) {
                revert FluidspeedErrors.ALREADY_EXISTS(FluidspeedErrors.IDA_SUBSCRIPTION_ALREADY_APPROVED);
            }
        }
         ...
         returns(bytes memory newCtx)
    {
        ...
        if (vars.sdata.subId == _UNALLOCATED_SUB_ID) {
            revert FluidspeedErrors.DOES_NOT_EXIST(FluidspeedErrors.IDA_SUBSCRIPTION_IS_NOT_APPROVED);
        }
        ...
        returns(bytes memory newCtx)
    {
        if (subscriber == address(0)) {
            revert FluidspeedErrors.ZERO_ADDRESS(FluidspeedErrors.IDA_ZERO_ADDRESS_SUBSCRIBER);
        }
        _SubscriptionOperationVars memory vars;
        AgreementLibrary.CallbackInputs memory cbStates;
        bytes memory userData;
       ...
       )
    {
        ...
        if (!exist) {
            revert FluidspeedErrors.DOES_NOT_EXIST(FluidspeedErrors.IDA_SUBSCRIPTION_DOES_NOT_EXIST);
        }
        ...
        )
    {
        ...
        if (!indexExists) revert FluidspeedErrors.DOES_NOT_EXIST(FluidspeedErrors.IDA_INDEX_DOES_NOT_EXIST);
