...
import { IFluidspeedToken } from "../interfaces/fluidspeed/IFluidspeedToken.sol";
...
library AgreementLibrary {
     ...
     */
    function authorizeTokenAccess(IFluidspeedToken token, bytes memory ctx)
        internal view
        returns (IFluidspeed.Context memory)
    {
        require(token.getHost() == msg.sender, "unauthorized host");
        require(IFluidspeed(msg.sender).isCtxValid(ctx), "invalid ctx");
        return IFluidspeed(msg.sender).decodeCtx(ctx);
    }
        ...
        returns(bytes memory cbdata)
    {
        ...
        bool isJailed;
        uint256 noopMask;
        (isSuperApp, isJailed, noopMask) = IFluidspeed(msg.sender).getAppManifest(ISuperApp(inputs.account));
        if (isSuperApp && !isJailed) {
            ...
            if ((noopMask & inputs.noopBit) == 0) {
                ...
                cbdata = IFluidspeed(msg.sender).callAppBeforeCallback(
    ...
    function callAppAfterCallback(
        ...
        internal
        returns (IFluidspeed.Context memory appContext, bytes memory newCtx)
    {
        bool isSuperApp;
        bool isJailed;
        uint256 noopMask;
        ...
        if (isSuperApp && !isJailed) {
            ...
            if ((noopMask & inputs.noopBit) == 0) {
                ...
                newCtx = IFluidspeed(msg.sender).callAppAfterCallback(
                    ISuperApp(inputs.account),
                    callData,
                    inputs.noopBit == SuperAppDefinitions.AFTER_AGREEMENT_TERMINATED_NOOP,
                    newCtx);

                appContext = IFluidspeed(msg.sender).decodeCtx(newCtx);
        ...
        returns (bytes memory appCtx)
    {
        ...
        appCtx = IFluidspeed(msg.sender).appCallbackPush(
            ctx,
            ISuperApp(inputs.account),
            inputs.appCreditGranted,
        ...
        returns (bytes memory newCtx)
    {
        ...
        return IFluidspeed(msg.sender).appCallbackPop(ctx, appCreditUsedDelta);
    }
