...
import { IFluidspeedToken, FluidspeedToken } from "./FluidspeedToken.sol";
...
/**
 * @title Fluidspeed's super token implementation
 *
 * @author Fluidspeed
 */
    ...
    ISuperToken
{
    ...
    function proxiableUUID() public pure override returns (bytes32) {
        return keccak256("org.fluidspeed-finance.contracts.SuperToken.implementation");
    }

    function updateCode(address newAddress) external override {
        if (msg.sender != address(_host)) revert FluidspeedErrors.ONLY_HOST(FluidspeedErrors.SUPER_TOKEN_ONLY_HOST);
     ...
     * For more context, see https://github.com/fluidspeed-finance/protocol-monorepo/wiki/About-ERC-777
     */
    function _transferFrom(address spender, address holder, address recipient, uint amount)
        internal returns (bool)
    {
        if (holder == address(0)) {
            revert FluidspeedErrors.ZERO_ADDRESS(FluidspeedErrors.SUPER_TOKEN_TRANSFER_FROM_ZERO_ADDRESS);
        }
        if (recipient == address(0)) {
            revert FluidspeedErrors.ZERO_ADDRESS(FluidspeedErrors.SUPER_TOKEN_TRANSFER_TO_ZERO_ADDRESS);
        }
    ...
    function _move(
        ...
        private
    {
        FluidspeedToken._move(from, to, amount.toInt256());
        ...
        internal
    {
        if (account == address(0)) {
            revert FluidspeedErrors.ZERO_ADDRESS(FluidspeedErrors.SUPER_TOKEN_MINT_TO_ZERO_ADDRESS);
        }

        FluidspeedToken._mint(account, amount);
    ...
    {
        if (from == address(0)) {
            revert FluidspeedErrors.ZERO_ADDRESS(FluidspeedErrors.SUPER_TOKEN_BURN_FROM_ZERO_ADDRESS);
        }

        _callTokensToSend(operator, from, address(0), amount, userData, operatorData);

        FluidspeedToken._burn(from, amount);
    ...
    {
        if (account == address(0)) {
            revert FluidspeedErrors.ZERO_ADDRESS(FluidspeedErrors.SUPER_TOKEN_APPROVE_FROM_ZERO_ADDRESS);
        }
        if (spender == address(0)) {
            revert FluidspeedErrors.ZERO_ADDRESS(FluidspeedErrors.SUPER_TOKEN_APPROVE_TO_ZERO_ADDRESS);
        }
    ...
    /// @dev IFluidspeedGovernance.getUnderlyingToken implementation
